...
typedef struct 
{
    ...
    int32_t Turns_cnt;//当前圈数
    int32_t TarTurns_cnt;//目标圈数
    int32_t Turns_flag;
}RUD_Param_t;
...
private:
    ...
    void Set_MaxSpeed();                                    /*<! 速度限制 */
		void Sentry_Follow(float *Vx, float *Vy, float *Vw,uint8_t Yaw,uint16_t YAW_CENTRE);      /*<! 跟随模式 */
    void Follow_Ctrl(float *Vx, float *Vy, float *Vw,uint8_t Yaw,uint16_t YAW_CENTRE);      /*<! 取消跟随模式 */
    void ReFollow_Ctrl(float *Vx, float *Vy, float *Vw);    /*<! 反向跟随模式 */
    void Spin_Ctrl(float *Vx, float *Vy, float *Vw,uint8_t Yaw);        /*<! 自旋模式 */
...
public:
    ...
    int16_t ReF_Cnt;      /*<! 反向过度累计时间 */
     ...
     int16_t Target_Low_Vx, Target_Low_Vy, Target_Low_Vw;
//    Target_Low Low_speed;
//    Target_Up  Up_speed;
