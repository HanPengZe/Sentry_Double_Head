...
void CTRL_DR16_classdef::LeverMode_Update(void)
{
    ...
    case Lever_MID:  // --- 左中 ----------------------------------------------
        ...
        case Lever_UP:
            ...
            Infantry.Set_GimbalMode(Gimbal_Up);

						if(DR16.Get_DW_Norm() >= 50||DR16.Get_DW_Norm() <= -50)
            {
        ...
        case Lever_MID:
            Infantry.Set_GimbalMode(Gimbal_Low);
            Infantry.Set_ChassisMode(CHAS_FollowMode);
						if(DR16.Get_DW_Norm() >= 50||DR16.Get_DW_Norm() <= -50)
            {
                Infantry.Set_ChassisMode(CHAS_SpinMode);
...
void CTRL_DR16_classdef::RCCtrl_Update(void)
{
    ...
    case CHAS_SpinMode:
        ...
        Expt.Target_Vy = DR16.Get_LY_Norm() * 10.0f;
				Expt.Target_Vw = (Infantry.Get_CtrlSource() == CTRL_RC ? DR16.Get_DW_Norm() / 660 * 4500.0f : -8000);
