...
float RUD_InitAngle[4] = {45.0f, 135.0f, 100.0f, 8.0f};
...
{
//    Gimbal.Motor[Yaw_Low].encoder_offset = GIMBAL_YAW_CENTRE;
		if(Mode == CHAS_SentryPCMode)
		{
			if((Infantry.robot_state.pack.bullet_remain == 0)||(Infantry.robot_state.pack.my_output== 0))
			{			 
				Process(Target_Vx, Target_Vy, 4500);				
			}
			else
			{
				Process(Target_Vx, Target_Vy, 0);			
			}
		}
    else if(Mode != CHAS_LockMode)
    {
			 Process(CTRL_DR16.Expt.Target_Vx, CTRL_DR16.Expt.Target_Vy, CTRL_DR16.Get_ExptVw());
    }
    else
    {
        Process(0,0,0);
...
{
	if(Gimbal.Mode == Gimbal_PCMode)//使用上位机给的雷达数据
	{
		if((Infantry.robot_state.pack.bullet_remain == 0)||(Infantry.robot_state.pack.my_output== 0))
		{
		...
		{
        Sentry_Follow(&Vx, &Vy, &Vw,Yaw_Low,GIMBAL_YAW_CENTRE);
		}
	
	}
    //--- Yaw轴中心点
	else if(Gimbal.Mode==Gimbal_Low)//无跟随
	{
     Gimbal.Motor[Yaw_Low].encoder_offset = GIMBAL_YAW_CENTRE;
    switch(Infantry.Get_ChassisMode())
			{
			
    case CHAS_FollowMode: //--- 无跟随模式
        Follow_Ctrl(&Vx, &Vy, &Vw,Yaw_Low,GIMBAL_YAW_CENTRE);
        break;
    case CHAS_SpinMode: //--- 小陀螺
        Spin_Ctrl(&Vx, &Vy, &Vw,Yaw_Low);
        break;
			}
	}
	else if(Gimbal.Mode==Gimbal_Up)
	{
     Gimbal.Motor[Yaw_Up].encoder_offset = GIMBAL_YAW_CENTRE_UP;
    switch(Infantry.Get_ChassisMode())
			{
			
    case CHAS_FollowMode: //--- 无跟随模式
        Follow_Ctrl(&Vx, &Vy, &Vw,Yaw_Up,GIMBAL_YAW_CENTRE_UP);
        break;
    case CHAS_SpinMode: //--- 小陀螺
        Spin_Ctrl(&Vx, &Vy, &Vw,Yaw_Up);
        break;
			}
	}
	else{}
...
{   
    float Cur_encoder = rudder_motor->getencoder()/8192*360;//换算乘当前角度，0~360°
    static float Pre_encoder[4] = {0};

    if(Cur_encoder - Pre_encoder[motor_num] > 180)//过零处理
    {
        RUD_Param[motor_num].Turns_cnt--;
    }
    else if(Cur_encoder - Pre_encoder[motor_num] < -180)//过零处理
    {
        RUD_Param[motor_num].Turns_cnt++;
    }
    ...
    {
        RUD_Param[motor_num].Total_angle = Cur_encoder + RUD_Param[motor_num].Turns_cnt*360;//计算舵轮的总角度
    }
...
//    Constrain(&Low_speed.Target_Vx, (int16_t)(-VxVy_Limit), (int16_t)VxVy_Limit);
//    Constrain(&Low_speed.Target_Vy, (int16_t)(-VxVy_Limit), (int16_t)VxVy_Limit);
//    Constrain(&Low_speed.Target_Vw, (int16_t)(-Vw_Limit), (int16_t)Vw_Limit);

//    Constrain(&Up_speed.Target_Vx, (int16_t)(-VxVy_Limit), (int16_t)VxVy_Limit);
//    Constrain(&Up_speed.Target_Vy, (int16_t)(-VxVy_Limit), (int16_t)VxVy_Limit);
//    Constrain(&Up_speed.Target_Vw, (int16_t)(-Vw_Limit), (int16_t)Vw_Limit);
...
uint8_t flag_45;
//float offset_angle ;
void Chassis_classdef::Follow_Ctrl(float *Vx, float *Vy, float *Vw,uint8_t Yaw,uint16_t YAW_CENTRE)
{
...
//        ReF_Cnt = 0;
        ...
        Speed_Decompose(Gimbal.Get_YawDeviateC(Gimbal.Motor[Yaw].getAngle())+PI/2, Vx, Vy);//这里全向公式加PI/2是因为我要让我舵轮的正方向和头的正方向对齐

        if(fabs(Gimbal.Motor[Yaw].getencoder() - YAW_CENTRE) > 5 )
        {
            // follow pid calc 
            offset_angle = Gimbal.Motor[Yaw].getAngle();//距离yaw轴中心的偏差角度
//            if(offset_angle > 180)
//            {
//                offset_angle -= 360.0f;
//            }
...
//            Follow_PID[PID_Outer].Target = Gimbal.Get_YawDeviateC(offset_angle/* Gimbal.Motor[Yaw].getAngle() */);
...
void Chassis_classdef::Sentry_Follow(float *Vx, float *Vy, float *Vw,uint8_t Yaw,uint16_t YAW_CENTRE)
{
    static uint8_t done_cnt; // 过度完成计时
    static uint8_t warning_flag = false; // 底盘警告标志位
    static float Vw_temp = 0;
...
float Vw_coe = 1.0f;
void Chassis_classdef::Spin_Ctrl(float *Vx, float *Vy, float *Vw,uint8_t Yaw)
{
...
//    }
    ...
    Speed_Decompose(Gimbal.Get_YawDeviateC(offset_angle)+PI/2, Vx, Vy);
...
//    Speed_Decompose(Gimbal.Get_YawDeviateC(Gimbal.Motor[Yaw].getAngle()+Vision.Radar_Recv_Msg.Pack.chassis_forward), Vx, Vy);
		...
		temp_spin = RAMP_Output(*Vw, temp_spin, 60.0f);
		*Vw = temp_spin;
...
{
    return Referee.GameRobotState.chassis_power_limit; // 底盘最大功率
}
