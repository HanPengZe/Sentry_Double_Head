...
uint16_t Reset_cnt;
uint8_t Laser_mark;
uint8_t Laser_Finish;
VisionMode_e visiontemp = Vision_Default;
TickType_t Current_Time;
TickType_t Last_Time;
TickType_t interval_Time = pdMS_TO_TICKS(200);
        ...
        case Lever_UP: // --- PC端控制——哨兵自动
						if(Infantry.Get_CtrlSource() != CTRL_RC)
            {
                ...
                Infantry.Set_AttackMode(Attack_Disable);
						...
						Chassis.mode_WYT = 0;
//						Vision.Radar_Send_Msg.Pack.mode = 1;//守
...
//						Shoot.Rage_Mode = true;
						Vision.aim_flag = 1;
//						if(abs(Vision.Tar_Yaw_Vision-Gimbal.Get_IMUTotalAngle(Yaw) * ENCODER_ANGLE_RATIO)<105)
//						{
//							Vision.My_shoot=1;
//						}
//						else								
...
//						}
					...
					if(RV_slove_expert.enemy==1)//子健BUG
					{
							if((RV_slove_expert.auto_aim==1 && Vision.aim_flag))
							{
...
//								Vision.aim_flag = 0;
						...
						Chassis.mode_WYT = 1;
//						Vision.Radar_Send_Msg.Pack.mode= 0;//攻
						Infantry.Set_GimbalMode(Gimbal_LockMode);
            Infantry.Set_ChassisMode(CHAS_SentryPCMode);
            Infantry.Set_VisionMode(Vision_Forcast);
						Infantry.Set_AttackMode(Attack_Zero); //让摩擦轮开启
						Vision.aim_flag = 0;
        ...
        case Lever_DOWN:	// — — 哨兵打弹 
            if(Infantry.Get_CtrlSource() != CTRL_RC)
            {
                ...
                Infantry.Set_AttackMode(Attack_Disable);
						...
						Infantry.Set_GimbalMode(Gimbal_PCMode);//巡逻
						Infantry.Set_ChassisMode(CHAS_LockMode);//雷达全向移动
						
            Infantry.Set_VisionMode(Vision_Forcast);
						Infantry.Set_AttackMode(Attack_30); //让摩擦轮开启
						Vision.aim_flag = 0;
					if(DR16.Get_DW_Norm() <=-400&&Laser_Finish==1)//1.按下且上一次的按键已经松开
					{
						
						if(Laser_mark == 0)			 //2.激光如果未开启
						{
							Shoot.Laser_Ctrl(ON);  // 打开激光
							Laser_Finish=0;				 //标志按键还没有松开不会重复进行
							Laser_mark=1;					 //激光标志位开启
						}												 //跳出
						else if(Laser_mark==1)
						{
							Shoot.Laser_Ctrl(OFF); // 关闭激光
							Laser_Finish = 0;      // 标志按键还没有松开不会重复进行
							Laser_mark = 0;        // 激光标志位关闭
						}
											
					}
					else if(DR16.Get_DW_Norm() >=-300)//3.按键松开，完成标志位置1
					{
						Laser_Finish=1;
					}
						if(DR16.Get_DW_Norm() > 450)
						{
							Vision.aim_flag = 1;
						}
						else
...
//        Infantry.Set_GimbalMode(Gimbal_NormalMode);
        ...
        case Lever_UP://无自瞄手动发射+跟随
            ...
            Infantry.Set_GimbalMode(Gimbal_LockMode);
            //--- 发射使能
						Infantry.Set_AttackMode(Attack_Zero);//测试拨盘失能摩擦轮
...
//						}
        ...
        case Lever_MID:
            ...
            if(DR16.Get_DW_Norm() >= 50||DR16.Get_DW_Norm() <= -50) //--- 拨上小陀螺
            {
    ...
    case Lever_DOWN: // --- 左下 ----------------------------------------------
        ...
        switch(DR16.Get_S2_R())
          ...
          Infantry.Set_GimbalMode(Gimbal_LockMode);
					...
					case Lever_MID :
//					Vision.aim_flag = 1;
					Infantry.Set_AttackMode(Attack_30);//测试拨盘失能摩擦轮
          Infantry.Set_GimbalMode(Gimbal_NormalMode);
					if(DR16.Get_DW_Norm() <=-400&&Laser_Finish==1)//1.按下且上一次的按键已经松开
					{
...
//						if(DR16.Get_DW_Norm() <=-600&&(Current_Time-Last_Time)>interval_Time)
//						{
//							if(Laser_mark == 0)
//							{
//								Shoot.Laser_Ctrl(ON);  // 打开激光
//								Laser_mark = 1;        // 更新激光状态标记							
//							}
//							else
//							{
//								Shoot.Laser_Ctrl(OFF);  // 打开激光
//								Laser_mark = 0;        // 更新激光状态标记							
//							}
//						}
...
//        break;
//    }
			switch(Infantry.Get_GimbalMode())
			{
				...
				case Gimbal_NormalMode:
					Expt.Target_Yaw =DR16.Get_RX_Norm()* Gimbal.YawRCCtrl_Coe ;
				  Expt.Target_Pit =DR16.Get_RY_Norm()*Coe.Pit_RC ;
					break;
