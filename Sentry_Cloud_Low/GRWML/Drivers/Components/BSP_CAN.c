...
uint8_t CANx_SendData_Current(CAN_HandleTypeDef* hcan, uint16_t ID, uint16_t Yaw, uint8_t Len)
{
	static uint8_t Yaw_Send_Out[8];
	static CAN_TxHeaderTypeDef Tx_Header;
	uint32_t used_mailbox;
	assert_param(hcan != NULL);
	Tx_Header.StdId = ID;
	...
	Yaw_Send_Out[6] = 0;
	Yaw_Send_Out[7] = 0;
	if (HAL_CAN_AddTxMessage(hcan, &Tx_Header, Yaw_Send_Out, &used_mailbox) != HAL_OK)
	{
		return CAN_LINE_BUSY;
	}
	else {}
