...
// --- 单次卡弹时间上限
#define Limit_StuckDuration 240
// --- 拨盘单个齿轮的角度
...
{
    Reload_PID[PID_Outer].SetPIDParam(0.5f, 0.0f, 0.0f, 10000, 10000, 0.002f);
    Reload_PID[PID_Inner].SetPIDParam(12.5f, 13.0f, 0.0f, 500, 9999, 0.002f);
    ...
    Reload.CriticalVal = 1300;
		Freq_PID.SetPIDParam(2.8f, 1.0, 2.1f, 5000, 4000, 0.20f);
		Freq_PID.I_SeparThresh = 9000;
...
//	{
//		if((Sentry_Recv_Msg_usb.Pack.auto_aim==1 && Vision.aim_flag)||(Vision.My_shoot==1))
//		{
//			ContLaunch = true;
...
//	else
//	{
//		ContLaunch = false;//（本来未加）子健BUG,没有敌人的时候lacunch会保留最后一次有敌人时的launch的值，使用有可能launch是ture
...
//		}
    ...
    {
        //set init angle...
				Send_Motor[SendID_Reload].Out = Reload_Motor.Out = 0;				
...
uint8_t Shoot_classdef::ShootFreq_Calc()
{
	...
	Freq_PID.Target = Infantry.shooter_barrel_heat_limit-60;
	if(Infantry.shooter_barrel_heat_limit>400)
	{
		Shoot_Freq = 1000;
	}
	else
	{	Freq_PID.Current = Infantry.shooter_id1_17mm_heat;
			Shoot_Freq = Freq_PID.Cal();
			if(Shoot_Freq<=0)Shoot_Freq = 0;
			else if(Shoot_Freq>=540)Shoot_Freq = 540;
			else {}//{Shoot_Freq=Shoot_Freq;}
	}
#else
		Shoot_Freq = 1000;
#endif
    ...
    if(Reload.Num > 0)
    {
        ...
        if(Shoot_Freq <= 0)
        {
            ...
            return;
				...
				//视觉发射的特殊限制
				//子健BUG，如果只有(Vision.Sentry_Send_Msg_usb.Pack.auto_aim==0 && Vision.aim_flag == 1)当launch为1的时候那全看Vision.Sentry_Send_Msg_usb.Pack.auto_aim是否为0；
//	if(Infantry.Get_GimbalMode() == Gimbal_PCMode)
//	{		
		if((RV_slove_expert.auto_aim==0 && Vision.aim_flag == 1))
				{
					Reload.Num = 0;
				}
//	}
...
//	else
//	{
//				if((Vision.Sentry_Send_Msg_usb.Pack.auto_aim==0 && Vision.aim_flag == 1)||Vision.Sentry_Send_Msg_usb.Pack.enemy==0||ContLaunch == false)
//				{
//					Reload.Num = 0;
//				}
...
//						Reload.Began2 = false;
        ...
        else
        {
            ...
            Reload.Launched_Num = abs(Reload_Motor.get_totalencoder() - Reload.Init_Angle) / GearTeeth_Angle;
				...
				if(Reload.Began2 == true && Reload.Began == true&&abs(Reload_Motor.get_totalencoder()-Reload.Target_Angle)<=32764/4) //*///&& abs(Reload_Motor.get_totalencoder() - (Reload.Target_Angle+32764/4)) <= 8192)
				{
						ShootOverTiming[Turn_Bar_ID]=HAL_GetTick();
						ShootTime[Turn_Bar_ID] = ShootOverTiming[Turn_Bar_ID]-ShootStartTiming[Turn_Bar_ID];
//						CanShoot_Num[Turn_Bar_ID]--;
            ...
            Reload.Stuck_Duration = 0;
				...
				if(Judge_ReloadStall() == true)
				{
					...
					if((Reload.Stuck_Duration > Limit_StuckDuration)&&mark==0)//卡弹在1300转速以内停滞120次
					{
						Reload.Target_Angle = Reload_Motor.get_totalencoder()- GearTeeth_Angle/2;			
						Reload.Stuck_Duration = 0;
						mark=1;
					}					
				}
    ...
    ReloadPID_Calc(Reload.Target_Angle);//下面的是拨盘电机
}
...
void Shoot_classdef::AttackMode_Ctrl()
{
    ...
    case Attack_30:
        ...
        else if((Mshoot_Cnt %= SHOOT_FREQ(Shoot_Freq)) == 0 && ContLaunch == true)//如果launch是ture那么就会进去，然后num就会置1，发射与否全看
        {																																					//Sentry_Send_Msg_usb.Pack.auto_aim是否为1
            Set_ReloadNum(1);
        }
			break;
    ...
    case Attack_Unload:
        if((Mshoot_Cnt%=SHOOT_FREQ(Shoot_Freq)) == 0 && ContLaunch == true)
        {
            Set_ReloadNum(1);
        }
        break;
...
{
    Shoot.Send_Motor[SendID_Reload].Out = Reload_Motor.Out = 0;
...
void Shoot_classdef::ReloadPID_Calc(float tar_angle)
{
    ...
    Send_Motor[SendID_Reload].Out = Reload_Motor.Out = Reload_PID[PID_Inner].Cal();
}
