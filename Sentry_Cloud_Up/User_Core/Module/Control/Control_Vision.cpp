...
/* Private macros ------------------------------------------------------------*/
// --- 是否使用卡尔曼
#define USE_YAW_KALMAN_PREDICT  1
#define USE_PIT_KALMAN	1
...
//	//--- 硬件波动会出现Nan值!!!
//	if(Sentry_Recv_Msg_usb.Pack.header != 0xA5)
//	{
//		crc_jiajia++;
//		Sentry_Recv_Msg_usb.Pack.yaw = Sentry_Recv_Msg_usb.Pack.pit /* = Recv_Msg.Pack.depth */ = 0;
//		Sentry_Recv_Msg_usb.Pack.enemy = Sentry_Recv_Msg_usb.Pack.auto_aim = Update_Flag = false;
//		return;
//	}
//	
//	if(Sentry_Recv_Msg_usb.Pack.yaw<30 && Sentry_Recv_Msg_usb.Pack.yaw>-30)
//	{
//		
//	}
//	else
//	{
//		Sentry_Recv_Msg_usb.Pack.enemy = Sentry_Recv_Msg_usb.Pack.auto_aim = Update_Flag = false;
//		Sentry_Recv_Msg_usb.Pack.pit = 0;
//		Sentry_Recv_Msg_usb.Pack.yaw = 0;
//		return;
//	}
//	if(Sentry_Recv_Msg_usb.Pack.pit<30 && Sentry_Recv_Msg_usb.Pack.pit>-30)
//	{
//		
//	}
//	else
...
//	}
//	
//	Sentry_Recv_Msg_usb.Pack.yaw = Sentry_Recv_Msg_usb.Pack.yaw;
//	Sentry_Recv_Msg_usb.Pack.pit = Sentry_Recv_Msg_usb.Pack.pit;
...
//	Tar_Yaw_Vision = (Gimbal.Get_IMUTotalAngle(Yaw)+ Sentry_Recv_Msg_usb.Pack.yaw)*ENCODER_ANGLE_RATIO;//在总角度的基础上加上视觉给的偏差值
//	Tar_Pit_Vision = (Gimbal.Get_IMUAngle(Rol)+ Sentry_Recv_Msg_usb.Pack.pit)*ENCODER_ANGLE_RATIO;//在总角度的基础上加上视觉给的偏差值

//	//敌人过滤处理
//	switch(Sentry_Recv_Msg_usb.Pack.enemy_id)
//	{
//		case 0://前哨站
//			if(Radar_Gimbal_Mode!=2){Sentry_Recv_Msg_usb.Pack.enemy=0;}
//		break;
//		case 1:if(Infantry.God_State[0]){Sentry_Recv_Msg_usb.Pack.enemy=0;}break;
//		case 2:if(Infantry.God_State[1]){Sentry_Recv_Msg_usb.Pack.enemy=0;}break;
//		case 3:if(Infantry.God_State[2]){Sentry_Recv_Msg_usb.Pack.enemy=0;}break;
//		case 4:if(Infantry.God_State[3]){Sentry_Recv_Msg_usb.Pack.enemy=0;}break;
//		case 5:if(Infantry.God_State[4]){Sentry_Recv_Msg_usb.Pack.enemy=0;}break;
//		case 6://哨兵
//			//等哨兵无敌解除
//			if(Infantry.Enemy_Sentry_God_State)
//			{
//				Sentry_Recv_Msg_usb.Pack.enemy=0;
//			};
//		break;
//		
//		case 7://基地
...
//	}
//	
//	if(Sentry_Recv_Msg_usb.Pack.enemy==1)//子健BUG
//	{
//		if(Sentry_Recv_Msg_usb.Pack.auto_aim==1 && aim_flag)
//		{
...
//	else
//	{
//		Shoot.ContLaunch = false;//（本来未加）子健BUG,没有敌人的时候lacunch会保留最后一次有敌人时的launch的值，使用有可能launch是ture
//		Sentry_Recv_Msg_usb.Pack.auto_aim=0;
//		Shoot.Set_ReloadNum(0);
//	}
//}
#else
...
void Vision_classdef::RecvFromPC(uint8_t *data, uint16_t ReceiveLen)
{
 ...
 float cunchu[5];
float XiangTongTime;
uint8_t ssss_buff[20];
void Vision_classdef::Radar_RecvFromPC(uint8_t *data, uint16_t ReceiveLen)
{
	DevicesMonitor.Get_FPS(&TIME, &FPS);

	memcpy(ssss_buff,data,sizeof(Radar_Recv_Msg.data));
	if(Radar_Recv_Msg.Pack.start != 0x05 || Radar_Recv_Msg.Pack.end != 0x06)
	{
...
void Vision_classdef::Sentry_SendToPC(Sentry_VisionSendMsg_Usb *pack2vision)
{
	...
	pack2vision->Pack.header = 0X05;
	// pack2vision->Pack.crc = crc_calc;
	pack2vision->Pack.detect_color =(!Infantry.Get_TeamColor());
...
//	pack2vision->Pack.checksum = Get_CRC16_Check_Sum(pack2vision->data, 14, CRC16_INIT);
	pack2vision->Pack.aim_x = RV_slove_expert.aim_x;
	pack2vision->Pack.aim_y = RV_slove_expert.aim_y;
	pack2vision->Pack.aim_z = RV_slove_expert.aim_z;
...
CDC_Transmit_FS(pack2vision->data,VISION_Sentry_TX_SIZE);

		/*
		HAL_UART_Transmit_DMA(&huart6 ,pack2vision->data, VISION_Sentry_TX_SIZE);//取消串口发送给视觉的数据
	*/
}
#else
/**
...
{
	Radar_Send_Msg.Pack.start = 0x06; 
#if SendRadar_Mode ==RMUC
//	Radar_Send_Msg.Pack.func = 'B'; // 'B'
//	Radar_Send_Msg.Pack.mode = 0; // 'S'
#endif

	Radar_Send_Msg.Pack.end = 0x09; // 'E'
//	for(int i=0;i<VISION_Radar_TX_SIZE;i++)
//	{
////		Radar_Send_Msg.data[i]=0;
//		HAL_UART_Transmit(&huart6, &Radar_Send_Msg.data[i], 1, 0xff);	//串口传输
//	}
//	test = HAL_UART_Transmit_DMA(&huart1 ,Radar_Send_Msg.data,  sizeof(Radar_Send_Msg.Pack));
		test=HAL_UART_Transmit(&huart1,Radar_Send_Msg.data,sizeof(Radar_Send_Msg.Pack),1000);
...
{
	return RV_slove_expert.enemy;
}
uint8_t Vision_classdef::IsAutoAim()
{
	return RV_slove_expert.auto_aim;
}
#else
...
{
	return RV_slove_expert.yaw;
}
float Vision_classdef::Get_PitOffset()
{
	return RV_slove_expert.pitch;
}
