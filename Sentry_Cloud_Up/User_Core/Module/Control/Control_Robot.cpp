...
//		MotorMsgSend(&hcan1, Gimbal.Send_Motor);//Gimbal[0]发送给云台Yaw轴，Gimbal[1]是拨盘
		MotorMsgSend(&hcan1,Gimbal.Motor_Yaw);
		MotorMsgSend(&hcan2, Shoot.Send_Motor); /*pit-FL-FR-Reload */
//		MotorMsgSend(&hcan2, Shoot.Card_Motor);
//		CANx_SendData_Current(&hcan1,0x1FE,Gimbal.Send_Motor[0].Out,8);
    //--- 发送数据至底盘主控
    SendMsgtoCHAS();//只有开启导航的时候才开始发送三轴速度
...
void Robot_classdef::RefereeMsg_Write(uint16_t id, uint8_t can_rx_data[])
{
    ...
    case 0x341:
			...
			shooter_barrel_heat_limit = robot_state.pack.shooter_barrel_heat_limit;
			shooter_id1_17mm_heat = robot_state.pack.shooter_17mm_2_barrel_heat;
			TeamColor = robot_state.pack.team_color;
			Vision.Radar_Send_Msg.Pack.bullet_remain = robot_state.pack.bullet_remain;
			Vision.Radar_Send_Msg.Pack.game_progress = robot_state.pack.game_progress;
			Vision.Radar_Send_Msg.Pack.game_progress_remain = robot_state.pack.stage_remain_time;
			Vision.Radar_Send_Msg.Pack.sentry_hp = robot_state.pack.sentry_hp;
			break;
		  ...
		  DR16.DataPack.S2_R  = (SW_Status_Typedef)Rev_DR16.pack.SR    ;			
			Vision.Radar_Send_Msg.Pack.red_outpost_hp = Rev_DR16.pack.red_outpost_hp;
			Vision.Radar_Send_Msg.Pack.blue_outpost_hp = Rev_DR16.pack.blue_outpost_hp;
			break;
			
		case 0x666:
				uint8_t data[4];
        float *Out;       //C板Yaw轴输出值
				data[0] = can_rx_data[0];
				data[1] = can_rx_data[1];
				data[2] = can_rx_data[2];
				data[3] = can_rx_data[3];
				Out = (float*)(&data);
				Gimbal.Motor_Yaw[Yaw_Low].Out = *Out;
		break;
